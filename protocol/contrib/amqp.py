...
class AMQPShortField(Field):
    ...
    def i2m(self, pkt, x):
        return b'\x60' + struct.pack('>H', x)

    def m2i(self, pkt, x):
        if x[0] == 0x60:
            return struct.unpack('>H', x[1:2])[0], 3
...
class AMQPLongField(Field):
    ...
    def i2m(self, pkt, x):
        ...
        if x == 0:
            ...
            ret = b'\x53' + struct.pack('>B', x)
        elif (x > 255) and (x <= 0xffffffffffffffff):
            ret = b'\x80' + struct.pack('>Q', x)
    ...
    def m2i(self, pkt, x):
        ...
        if mark == 0x44:
            ...
            return struct.unpack('>Q', x[1:9])[0], 9
...
class AMQPSignedIntField(Field):
    ...
    def i2m(self, pkt, x):
        ...
        if (x >= 0) and (x <= 255):
            ret = b'\x54' + struct.pack('>b', x)
        elif (x > 255) and (x <= 0xffffffff):
            ret = b'\x71' + struct.pack('>i', x)
...
class AMQPSignedLongField(Field):
    ...
    def i2m(self, pkt, x):
        ...
        if (x >= 0) and (x <= 255):
            ret = b'\x55' + struct.pack('>b', x)
        elif (x > 255) and (x <= 0xffffffffffffffff):
            ret = b'\x81' + struct.pack('>q', x)
...
class AMQPFloatField(Field):
    ...
    def i2m(self, pkt, x):
        return b'\x72' + struct.pack('>f', x)

    def m2i(self, pkt, x):
        mark = x[0]

        if mark == 0x72:
            return struct.unpack('>f', x[1:5])[0], 5
...
class AMQPDoubleField(Field):
    ...
    def i2m(self, pkt, x):
        return b'\x82' + struct.pack('>d', x)

    def m2i(self, pkt, x):
        mark = x[0]

        if mark == 0x82:
            return struct.unpack('>d', x[1:9])[0], 9
...
class AMQP(Packet):
    ...
    def post_build(self, p, pay):
        ...
        # FIXME: calculate doff instead of fixed 2
        total_len = len(p + pay)
